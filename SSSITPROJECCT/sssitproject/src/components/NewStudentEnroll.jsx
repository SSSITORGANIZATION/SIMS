...
export default function NewStudentEnroll() {
  ...
  const handleLastModified = () => {
    ...
    if (oldRecords.length === 0) {
      showMessage("No edited record found", "info");
      return;
    }
    ...
    setPreview(
      lastMod.image
        ? lastMod.image.startsWith("http")
          ? lastMod.image
          : `http://127.0.0.1:8000${lastMod.image}`
        : null
    );
  
    showMessage("Last modified record loaded", "success");
  };
        ...
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content p-3 text-center">
            ...
            <div className="d-flex justify-content-around mt-3">
              <button className="custom-btn custom-btn-blue" data-bs-dismiss="modal" onClick={() => setShowCamera(true)}>
                Camera
              </button>
              <button className="custom-btn custom-btn-green" data-bs-dismiss="modal" onClick={() => fileInputRef.current.click()}>
                Upload
              </button>
              {preview && (
                <button
