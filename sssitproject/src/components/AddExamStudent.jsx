...
const showMessage = (text, type = "success", timeout = 3000) => {
  ...
  const fetchStudents = async () => {
    try {
      const res = await axios.get(`http://127.0.0.1:8000/students/?t=${Date.now()}`);
  ...
  const handleUpdateAll = async () => {
    if (!fields.regNo || !fields.Exam_Course) {
     showMessage("Select Reg No and Exam Course", "warning");
return;
    }
...
return;
    ...
    try {
      await axios.patch(
        `http://127.0.0.1:8000/students/${student.id}/update_exam_course/`,
        payload,
        { headers: { "Content-Type": "application/json" } }
      );

      await fetchStudents();
      showMessage("Exam details updated successfully", "success");
...
showMessage("Failed to update exam details", "danger");
