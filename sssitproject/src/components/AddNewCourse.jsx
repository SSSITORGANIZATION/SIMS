...
function Add_new_course() {
  ...
  const fetchCourses = useCallback(async () => {
    try {
      const data = await makeApiCall(COURSE_URL);
  ...
  const handleAddCourse = async () => {
    ...
    try {
      await makeApiCall(COURSE_URL, "POST", { name: courseName.trim() });
      setCourseName("");
      fetchCourses();
    } catch (err) {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      await makeApiCall(`${COURSE_URL}${editingCourse.id}/`, "PUT", {
        name: editName.trim(),
      });
  ...
  const handleConfirmDelete = async () => {
    try {
      await makeApiCall(`${COURSE_URL}${deletingCourse.id}/`, "DELETE");
