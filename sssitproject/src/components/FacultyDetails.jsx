...
function Faculty_details() {
  const FACULTY_URL = "http://127.0.0.1:8000/faculty/";
  const BATCH_URL = "http://127.0.0.1:8000/batches/";
  const COURSE_URL = "http://127.0.0.1:8000/courses/";
  ...
  const fetchFaculty = async () => {
    try {
      const res = await axios.get(FACULTY_URL);
  ...
  const loadSubjectsFromApi = async () => {
    try {
      ...
      const names = (res.data || []).map((c) => c.course_name || c.name).filter(Boolean);
...
const sorted = [...data].sort((a, b) => b.id - a.id);
  ...
  const saveNewFaculty = async () => {
    try {
      ...
      await axios.post(FACULTY_URL, payload);
  ...
  const confirmDeleteFaculty = async () => {
    ...
    try {
      await axios.delete(`${FACULTY_URL}${selectedFaculty.id}/`);
  ...
  const deleteBatch = async (id) => {
    try {
      await axios.delete(`${BATCH_URL}${id} /`);
  ...
  const saveBatch = async () => {
    try {
      if (!selectedFaculty) {
      showMessage("Please select faculty first", "warning");
      return;
    }

     if (!selectedSubject) {
  showMessage("Please select subject", "warning");
  return;
}
  ...
  label,
      ...
      showMessage("Batch added successfully", "success"); // ✅ ADD
  ...
  const saveEditedBatch = async () => {
    try {
      ...
      await axios.put(`${BATCH_URL}${editBatch.id}/`, {
  faculty: selectedFaculty.id,
...
showMessage("Batch updated successfully", "success"); // ✅ ADD
...
loadBatches(selectedFaculty.id);
  ...
  const confirmDeleteBatch = async () => {
    try {
     await axios.delete(`${BATCH_URL}${batchToDelete.id}/`);

showMessage("Batch deleted successfully", "success"); // ✅ ADD
...
const showMessage = (text, type = "success", timeout = 3000) => {
    ...
    <div className="container">
      {message.text && (
        <div className={`alert-message slide-in ${message.type}`}>
          <i className={`bi ${
            message.type === "success" ? "bi-check-circle-fill" :
