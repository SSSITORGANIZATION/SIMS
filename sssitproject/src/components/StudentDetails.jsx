...
export default function StudentDetails() {
  ...
  const loadBreaks = async (student) => {
    const res = await axios.get(`${API_URLS.breaks}?student=${student.id}`);
    setBreaks(res.data);
  };
  ...
  const handleRegNoChange = (value) => {
    ...
    const timeout = setTimeout(() => fetchList(value), 400);
  ...
  const submitBreak = async () => {
    ...
    try {
      if (editMode) {
        await axios.put(`${API_URLS.breaks}${editId}/`, {
          student: selected.id,
          from_date: fromDate,
          to_date: toDate,
          reason,
        ...
        await axios.post(`${API_URLS.breaks}`, {
          student: selected.id,
          from_date: fromDate,
          to_date: toDate,
          reason,
      ...
      console.log("Error submitting break");
    }
  };
  ...
  const handleDelete = async (id) => {
    await axios.delete(`${API_URLS.breaks}${id}/`);
    await loadBreaks(selected);
  };
